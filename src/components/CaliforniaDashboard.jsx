...
const DASH_TEXT = {
  en: {
    ...
    available: "Available",
    summary: "Summary",
    summary_desc: "Average R² across all 15 crops improved from 0.45 to 0.82 after PEST calibration.",
  ...
  vi: {
    ...
    available: "Có sẵn",
    summary: "Tổng kết",
    summary_desc: "Chỉ số R² trung bình trên 15 loại cây tăng từ 0.45 lên 0.82 sau khi hiệu chỉnh PEST.",
...
const CROPS = [
  { id: 'rice', name: { en: 'Rice (aglivc)', vi: 'Lúa (aglivc)' }, r2: 0.82, rmse: 145.2, color: 'text-emerald-600', bg: 'bg-emerald-100' },
  { id: 'corn', name: { en: 'Corn (grain)', vi: 'Ngô (hạt)' }, r2: 0.91, rmse: 89.5, color: 'text-yellow-600', bg: 'bg-yellow-100' },
  { id: 'almond', name: { en: 'Almonds', vi: 'Hạnh nhân' }, r2: 0.78, rmse: 210.1, color: 'text-orange-600', bg: 'bg-orange-100' },
  { id: 'grapes', name: { en: 'Grapes', vi: 'Nho' }, r2: 0.85, rmse: 112.3, color: 'text-purple-600', bg: 'bg-purple-100' },
  { id: 'cotton', name: { en: 'Cotton', vi: 'Bông vải' }, r2: 0.88, rmse: 95.4, color: 'text-slate-600', bg: 'bg-slate-100' },
];
          ...
          <div className="lg:w-3/4 space-y-6">
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="flex items-end gap-2 mt-1"><span className="text-2xl font-bold text-slate-800">{isCalibrated ? selectedCrop.r2 : (selectedCrop.r2 * 0.6).toFixed(2)}</span><span className="text-xs text-emerald-600 mb-1 bg-emerald-50 px-1.5 py-0.5 rounded flex items-center"><TrendingUp size={10} className="mr-1"/> {isCalibrated ? '+35%' : '-'}</span></div>
              </div>
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <span className="text-xs text-slate-500 font-medium">{t.error}</span>
                <div className="flex items-end gap-2 mt-1"><span className="text-2xl font-bold text-slate-800">{isCalibrated ? selectedCrop.rmse : (selectedCrop.rmse * 1.5).toFixed(1)}</span><span className="text-xs text-rose-600 mb-1 bg-rose-50 px-1.5 py-0.5 rounded">kg C/ha</span></div>
              </div>
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              ...
              <div className="lg:col-span-2 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-4 text-xs">
                     <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-slate-800"></div> Obs</div>
                     <div className="flex items-center gap-1"><div className={`w-3 h-1 ${isCalibrated ? 'bg-emerald-500' : 'bg-slate-400 border-t border-dashed border-slate-600'}`}></div> Sim</div>
                  </div>
                </div>
